...
const roleRows = {
  user: { id: 'role-user', slug: 'user', name: 'Standard User', rank: 10 },
  admin: { id: 'role-admin', slug: 'admin', name: 'Admin', rank: 20 },
  superAdmin: { id: 'role-super', slug: 'super_admin', name: 'Super Admin', rank: 30 }
}
...
const sessionUser = {
  ...
  permissions: [
    ...
    { action: 'read', subject: 'Role', conditions: null }
  ],
...
const userRows = [
  { id: 'admin-id', email: 'admin@example.com', role: roleRows.admin, createdAt: '2026-01-01T00:00:00Z', updatedAt: '' },
  { id: 'other-admin-id', email: 'admin2@example.com', role: roleRows.admin, createdAt: '2026-01-01T00:00:00Z', updatedAt: '' },
  { id: 'user-id', email: 'user@example.com', role: roleRows.user, createdAt: '2026-01-01T00:00:00Z', updatedAt: '' }
]
...
describe('usersView', () => {
  beforeEach(() => {
    ...
    api.GET.mockImplementation(async (url: string) => {
      ...
      if (url === '/api/v1/admin/roles/')
        return { data: Object.values(roleRows).map(role => ({ ...role, permissions: [] })), response }
      return {
        data: {
          data: userRows,
  ...
  it('lists users and only shows controls for manageable rows', async () => {
    ...
    expect(wrapper.text()).toContain('admin2@example.com')
    expect(wrapper.text()).toContain('user@example.com')

    // admin actor: no controls on self or the other admin, controls on the standard user
    expect(wrapper.find('[aria-label="Role for admin@example.com"]').exists()).toBe(false)
    expect(wrapper.find('[aria-label="Role for admin2@example.com"]').exists()).toBe(false)

...
import { sessionQuery, useAppAbility } from '@/features/auth'
import { useAdminUserMutations } from '@/features/users/mutations'
import { adminRolesQuery, adminUsersQuery } from '@/features/users/queries'
...
const actor = computed(() => session.data.value)
const manageAll = computed(() => ability.value.can('manage', 'all'))
...
function canManage(user: AdminUser) {
  ...
  if (!self || user.id === self.id)
    return false
  return manageAll.value || self.role.rank > user.role.rank
}
...
function roleItems(user: AdminUser) {
  ...
  if (!self)
    return []
  const all = roles.data.value ?? []
  const assignable = manageAll.value ? all : all.filter(role => self.role.rank > role.rank)
  const options = assignable.some(role => role.slug === user.role.slug)
    ? assignable
    : [...assignable, user.role]
  return options.map(role => ({ label: role.name, value: role.slug }))
}

function onRoleChange(user: AdminUser, role: Role['slug']) {
      ...
      >
        ...
        <template v-if="canManage(user)">
          <USelect
            ...
            :model-value="user.role.slug"
            :items="roleItems(user)"
            :disabled="pending"
            class="w-40"
            @update:model-value="onRoleChange(user, $event as Role['slug'])"
          />
          <UButton

...
describe('admin user routes', () => {
  ...
  it('enforces the role-management matrix', async () => {
    ...
    expect(promote.json()).toMatchObject({ id: standard.id, role: { slug: 'admin' } })
    ...
    expect(demote.json()).toMatchObject({ role: { slug: 'user' } })
  ...
  it('reads the role fresh from the database on every request', async () => {
    ...
    await db.update(users).set({ roleId: await roleId('user') }).where(eq(users.id, admin.id))

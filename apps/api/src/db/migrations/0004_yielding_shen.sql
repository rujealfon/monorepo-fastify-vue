...
INSERT INTO "roles" ("name", "description", "is_system") VALUES
	...
	('super_admin', 'Super administrator', true);--> statement-breakpoint
INSERT INTO "role_permissions" ("role_id", "permission")
	SELECT r.id, p FROM "roles" r, unnest(ARRAY['users:read', 'users:manage', 'roles:read']) AS p WHERE r.name = 'admin';--> statement-breakpoint
ALTER TABLE "users" ADD COLUMN "role_id" uuid;--> statement-breakpoint
UPDATE "users" SET "role_id" = r.id FROM "roles" r WHERE r.name = "users"."role"::text;--> statement-breakpoint

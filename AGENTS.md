...
Configure `apps/api/.env.test` with a separate test database before running `pnpm test`. Do not commit `.env` or `.env.test`; start from the checked-in example files.
...
Module-owned Drizzle tables are composed in `apps/api/src/db/schema/index.ts`. Generate and review migrations with `pnpm db:generate`, and apply them with `pnpm db:migrate`. Never use schema push in production.

...
This is a pnpm workspace monorepo. The API lives in `apps/api`, the Vue/Vite application lives in `apps/app` (UI components from `@nuxt/ui`, not the Nuxt framework; composition utilities from `@vueuse/core`), the separate Nuxt site lives in `apps/site`, and shared packages live in `packages`. See `apps/api/CLAUDE.md`, `apps/app/CLAUDE.md`, and `apps/site/CLAUDE.md` for layout and coding conventions specific to each app.

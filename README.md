...
Also set `NUXT_PUBLIC_API_URL` to the API project's URL (e.g. `https://api.example.com`) — unlike web, site has no build/dev-time proxy to the API (`nuxt generate`'s static output has no server to proxy through), so the site's header calls this origin directly client-side to check login state and to log out (`apps/site/app/composables/use-profile.ts`), and to redeem a web→site handoff token when arriving with one (`apps/site/app/plugins/handoff.client.ts`, see [Direct requests between unrelated domains](#direct-requests-between-unrelated-domains) below). This only works once `CORS_ORIGIN` is set on the API project as described above; `COOKIE_DOMAIN` additionally makes it work without the handoff token when web and site share a registrable domain. Defaults to `http://localhost:3000` for local development.
...
Web and site have no such rewrite proxy between them (site calls the API directly, not through web), so on bare `*.vercel.app` project URLs the session cookie set on web is simply invisible to site — there's no shared `COOKIE_DOMAIN` to set (see above), and `SameSite=None` isn't a real option for the reasons just described. To let site still recognize a web session without a shared domain, `apps/api/src/modules/users` exposes a short-lived, one-time **handoff**: `POST /api/v1/auth/handoff` (authenticated, called from web) mints an opaque token stored in Redis for `HANDOFF_TOKEN_TTL_SECONDS`; `POST /api/v1/auth/handoff/exchange` (called from site) redeems it exactly once for a brand-new session — and cookie — scoped to site's own origin. Web's `AppLayout` mints a token and appends it to the URL whenever it links to site (`goToSite()`); site's `apps/site/app/plugins/handoff.client.ts` picks it up on load, exchanges it, and strips it from the address bar before the page renders. This requires `REDIS_URL` to be configured (already required in production for rate limiting) and is only ever a mitigation — once web and site share a registrable domain, set `COOKIE_DOMAIN` instead, since the cookie is then already visible to site directly. `COOKIE_DOMAIN` is the single source of truth for which mode is active: `mintHandoff` (`apps/api/src/modules/users/users.handlers.ts`) checks it server-side and returns `{ token: null }` without touching Redis when it's set, and web's `goToSite()` already treats a null token as "nothing to append" — so no separate flag needs to be kept in sync across the web and API Vercel projects, and no other code changes when switching between the two setups.

...
Vue components shared between `apps/app` (Vite + Vue Router) and `apps/site` (Nuxt).
...
`@nuxt/ui`'s `to`/`href` props already handle cross-framework links: an internal path (`/health`) renders through whichever router is present (`NuxtLink` in site, `RouterLink` in app), and an absolute URL (`https://...`) always renders a plain `<a>`. This lets one component serve links into the other app (e.g. app's `AuthLayout` linking `Home`/`About` to site) without any conditional logic — pass the right href from the consuming app and the component doesn't need to know which app it's in.
